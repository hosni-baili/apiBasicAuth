...
import com.sesame.amsrest.repository.ArticleRepository;
...
import javax.validation.Valid;
	...
	return articleRepository.save(article);
	 ...
	 return articleRepository.save(article);}).orElseThrow(() -> new ResourceNotFoundException("ArticleId " +
			 articleId + "not found")); }
 	...
 	public ResponseEntity<?> deleteArticle(@PathVariable (value = "articleId") Long articleId) {
 		...
 		return ResponseEntity.ok().build();}).orElseThrow(() -> new ResourceNotFoundException("Article not found with id " + articleId));
 	}
}
